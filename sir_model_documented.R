# SIR Model for Covid-19 Transmission in R
# -----------------------------------------
# This script implements a simple SIR (Susceptible-Infected-Recovered) epidemic model
# using ordinary differential equations (ODEs), parameterized by the basic reproduction number (R0).
#
...
# -----------------------------------------------------------------------------
# Plot epidemic curves: Susceptible, Infected, Recovered
# -----------------------------------------------------------------------------
plot(out$time, out$I, type = "l", col = "red", lwd = 2,
     ylab = "Number of people", xlab = "Time (days)",
     main = "SIR Model: Covid-19 Transmission")
lines(out$time, out$S, col = "blue", lwd = 2)
lines(out$time, out$R, col = "green", lwd = 2)
